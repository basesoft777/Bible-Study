...
# Karoli ures helyorzo sorok - ezeket ki kell zarni a fenti max-bol, ha vannak
ures_helyorzo = set()
if os.path.exists("konkordancia/Karoli_ures_helyorzo_sorok.tsv"):
...
if ures_helyorzo:
    # ujraszamoljuk a karoli_max-ot az ures-helyorzo sorok nelkul
    karoli_max = {}
    with open("konkordancia/Karoli_1908.tsv", encoding="utf-8") as f:
        f.readline()
        for line in f:
...
# KJV max vers/fejezet (verse_pairs.jsonl mt_refs, mt_book+fejezet kulccsal)
kjv_max = {}
...
# MT max vers/fejezet (TAHOT_kivonat.tsv, karoli konyvnevvel)
mt_max = {}
...
with open("naplok/KAROLI_KK1_fejezetosztaly.tsv", "w", encoding="utf-8") as out:
...
print(f"Osszesen {len(results)} fejezet, 39 konyv")
print(c)
egyik_sem = [r for r in results if r[6] == "EGYIK_SEM"]
print(f"\nEGYIK_SEM fejezetek ({len(egyik_sem)}):")
for r in egyik_sem:
    print(" ", r)
