...
*v1 — 2026.09.23 · jóváhagyásra: a §2 S-döntései és a §0/§4 számai · a `RENDER_BRIEF.md` v3 kettéválásából (ott D13); OCR-kezelés (D12), héber kiejtés-jelöltek (D13), LXX-korpuszszint (D14) · KIEJT-kiváltás (D15) · **v1.1 — 2026.09.25: a Cremer kivezetve (D16); forrásszabály: csak saját repóban tárolható és onnan renderelhető forrás (D17)***
...
TBESH egyesítése, az UBS DBH teljes importja, a Mounce/SECE, a Girdlestone és a
BDB-etimológia adatosítása. A végén a 8 törzscikk 5. szakaszában nincs `nincs adatosítva` cella, kivéve a görög teológiai szócikk sorát, amelynek nincs forrása (D16).
...
| 0.12 | `szotar_szerepek.tsv` | 20 sor (10 szerep × 2 nyelv; az LXX-híd adatosítva); `nincs adatosítva`: Cremer (v1.1-től a forrás: „nincs — D16”), Girdlestone + Cremer héber mutatója (v1.1-től: Girdlestone), UBS DBH (3 szerep), Mounce, SECE (2), BDB-etimológia, kiejtés (görög, héber) |
...
| S7 | Cremer (görög teológiai szócikk) — **kivezetve (D16)** | Nincs import és nincs render, sem szövegként, sem hivatkozásként. A `szotar_szerepek.tsv` gorog/3 sorának forrása: „nincs (Cremer kivezetve, SZOTAR D16)”, állapota `nincs adatosítva`. Az S0.2 eredménye (`naplok/SZOTAR_S0_cremer.tsv`) és a `CREMER_OCR_BRIEF.md` (v3, lezárva) naplóként megmarad; a korábbi S7-szöveg a git-történetben. |
| S8 | Girdlestone (héber teológiai szócikk) | **`konkordancia/Girdlestone_szocikkek.tsv`** (`fejezet`, `heber_lemma`, `strong`, `strong_parositas`, `oldal`, `szoveg`, `allapot`, `ocr_gyanus`), szócikk-szinten, csak a motívumok tokenjeire (ma 24 H). Forrás: gépelt átirat (preceptaustin.org PDF), tartalék az archive.org OCR — az S0.3 dönti el. OCR-kezelés, javítási napló (`konkordancia/Girdlestone_javitasi_naplo.tsv`) és render-szabály, mint az S7. A héber cella: Girdlestone; a TWOT-szám hivatkozásként marad (a D17 kivétele). A Cremer héber mutatója kivezetve (D16). |
...
| S11 | Lábléc és kolofon (a pilot D12-je kiegészül) | Mounce (kötelező megjelölés), SECE, Girdlestone (közkincs, kiadással), UBS DBH (CC BY-SA 4.0, a ©-mondat szó szerint). A ShareAlike a publikálást érinti (N11), a munkát nem blokkolja. |
...
- **S0.2** *(elvégezve; a Cremer ezután kivezetve, D16 — nem fut újra)* Cremer: szövegforrás-jelöltek (levendwater.org HTML, archive.org OCR) összevetése a 13 G-tokenen: hibaarány **külön a latin és külön a görög/héber betűs szövegre**, megmaradtak-e a görög betűk (vagy átírásra cserélődtek), szócikkhatárok; lefedettség a 13 G-tokenre. `naplok/SZOTAR_S0_cremer.tsv`. Ha a hálózat a letöltést nem engedi: **ÁLLJ** és jelents.
- **S0.3** Girdlestone: ugyanez a 24 H-tokenre (preceptaustin.org PDF, archive.org OCR), a héber betűk megmaradásával. `naplok/SZOTAR_S0_girdlestone.tsv`.
...
- **S1.4** Importok a `konkordancia/` alá, README-vel és licenccel: `TBESH_konszolidalt.tsv` (S4), `UBS_DBH_*` (S5), `MCGED_teljes.tsv` (S6), `Girdlestone_szocikkek.tsv` (S8), és ha az S0.8 engedi, `LXX_versszintu_parok.tsv` (S13). A generátor még nem olvassa őket.
- **S1.4b** Girdlestone: OCR-ellenőrzés (`ocr_gyanus`), kézi javítás a javítási naplóba, `allapot` → `javitott`. Összesítő: `naplok/SZOTAR_S1_ocr.tsv` (szócikkenként gyanús tételek, javítások száma).
...
- **ÁLLJ — jóváhagyás:** a javított Girdlestone-szócikkek (`javitott` → `jovahagyott`) és a 24 héber kiejtés-jelölt. A jóváhagyott állapot és a jóváhagyott jelöltek a 2. menet első commitjában kerülnek be.

### 2. menet — kimenet-változtató, elvárt diffel

- **S2.1** A jóváhagyott állapot rögzítése (Girdlestone `jovahagyott`; a 24 héber lemma a `kiejtes_kivetelek.tsv`-be). Görög kiejtés a generált blokkokban (lemma és alak), a tisztított átírás-lista szerint; héber lemma a kivétel-táblából. **A `torzscikk_general.py` kódbeli `KIEJT` táblája megszűnik**: a törzscikk is a `kiejtes.py`-ból és a kivételtáblából olvas, így a 8 törzscikk egységesen magyaros átírást kap (a pilot D10 konvenciója). Előfeltétel: 100%-os görög egyezés.
...
- **S2.5** Girdlestone (csak `jovahagyott` szöveg), BDB-etimológia (S8–S9, S10); ha az S1.4 importálta, az LXX versszintű párok (S13).
...
- **S2.9** Lezárás: `NYITOTT_FELADATOK.md` (fordítási pipeline, BDB SQLite-csere, héber gépi kiejtés, a 12 Thayer-szócikk és a Girdlestone-szövegek fordítása, Trench), `MUNKAMENET.md`.
...
| S0 | Girdlestone *(a Cremer S0.2-je elvégezve, utána kivezetve, D16)* | forrásonkénti hibaarány, külön latin és görög/héber betűs szövegre; lefedettség 13 G / 24 H |
| S0 | héber kiejtés-jelöltek | egyezés a 49 arany párral jelentve |
| 1 | Girdlestone-szócikkek | mind `javitott`, a gyanús tételek mind kezelve (javítva vagy indokoltan elfogadva) |
...
| 2 | a renderben használt Girdlestone-szöveg | csak `jovahagyott` |
...
| K7 | az importált táblák reprodukálhatók (importszkript + forrás-SHA a README-ben); a Girdlestone javításai a naplóból visszakövethetők |
...
| `S1.4: szótári importok` | `konkordancia/TBESH_konszolidalt.tsv`, `konkordancia/UBS_DBH_*.tsv`, `konkordancia/MCGED_teljes.tsv`, `konkordancia/Girdlestone_*.tsv`, `konkordancia/LXX_versszintu_parok.tsv` (ha van), importszkriptek, `konkordancia/README.md` |
| `S1.4b: Girdlestone OCR-javítás` | `konkordancia/Girdlestone_szocikkek.tsv`, `konkordancia/*_javitasi_naplo.tsv`, `naplok/SZOTAR_S1_ocr.tsv` |
...
| Üzenet | Fájlok |
|---|---|
| `S2.1: jóváhagyott szócikkek és héber kiejtések; görög kiejtés a generált blokkokban` | `konkordancia/Girdlestone_szocikkek.tsv`, `adat/kiejtes_kivetelek.tsv`, `eszkozok/lexikon_general.py`, `eszkozok/kiejtes.py`, `lexikon/*` |
| `S2.2–S2.5: TBESH, UBS DBH, Mounce/SECE, Girdlestone, BDB-etimológia a renderben` | `eszkozok/lexikon_general.py`, `eszkozok/torzscikk_general.py`, `adat/forditasok.tsv`, `lexikon/*` |
...
4. ÁLLJ: mutasd be a javított Girdlestone-szócikkeket (az ocr_gyanus
...
| D8 | *(visszavonva: D16)* Görög teológiai szócikk: Cremer (S7) | közkincs (1895); a TDNT a Cremer átdolgozásaként indult; klasszikus → LXX → ÚSZ útvonal, mint a projekt LXX-hídja |
| D9 | Héber teológiai szócikk: Girdlestone (a Cremer héber mutatója visszavonva, D16); a TWOT-szám csak hivatkozás (S8) | a TWOT szövege jogvédett, a repóban csak a száma van; a Girdlestone közkincs, tematikus, LXX-en át az ÚSZ-hez köt |
...
| D16 | A Cremer kivezetve: sem szövegként, sem hivatkozásként nem kerül a szótári rétegbe; a görög teológiai szócikk szerepnek nincs forrása; a D8 és a D12 Cremer-része visszavonva (S7) | A felhasználó döntése, 2026.09.25. A szöveg angol, fordítás nélkül nem kerül a magyar cikkbe; tartalmi hozzájárulását (klasszikus → LXX → ÚSZ jelentéstörténet) a TBESG, az Abbott-Smith és a LXX-adatok nagyrészt lefedik; a gépi javítás és a célzott kinyerés ráfordítása nem arányos (`CREMER_OCR_BRIEF.md` v3, D20–D22). Az Abbott-Smith szócikkeiben álló „Cremer, oldal” hivatkozás az Abbott-Smith szövegének része, változatlan marad |
| D17 | Forrásszabály: szótári forrás csak akkor kerül a szerepmátrixba és a renderbe, ha a tartalma a saját repóban tárolható és onnan renderelhető; csak hivatkozásként elérhető forrás (pl. NIDNTTE, NIDOTTE) nem kerül be | A felhasználó döntése, 2026.09.25: a render reprodukálható marad, és nem függ jogvédett művek hozzáférésétől. Kivétel: a D9 TWOT-száma marad — maga a szám a saját repóban tárolt, renderelhető azonosító; a jogvédett szócikkszöveg nem kerül be (a felhasználó döntése, 2026.09.25) |
